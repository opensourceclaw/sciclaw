...
export class AuthorityScorer {
    ...
    scoreAuthority(author, institution, citationCount) {
        ...
        const overall = credentialScore * this.credentialWeight +
    ...
    scoreCredentials(_author, credentials) {
        if (!credentials)
            return 0.3;
    ...
    scoreNameRecognition(name) {
        if (!name)
            return 0.3;
        const lower = name.toLowerCase();
        if (WELL_KNOWN_AUTHORS.includes(lower))
            return 1.0;
...
export function scoreAuthority(author, institution, citationCount) {
    const scorer = new AuthorityScorer();
    return scorer.scoreAuthority(author, institution, citationCount);
}
//# sourceMappingURL=authority_scorer.js.map