 ...
 */
export function mockSearchResults(query, limit = 5) {
    const results = [];
...
export async function searchSubQueries(subQueries, opts) {
    const sets = [];
    for (const sq of subQueries.slice(0, opts.maxQueries)) {
        ...
        if (opts.mock) {
            ...
            const engines = (sq.sources.length ? sq.sources : undefined);
...
function extractSentences(results, max = 5) {
    const out = [];
...
export function extractiveSynthesis(query, results) {
...
const SYNTHESIS_SYSTEM_PROMPT = "You are SciClaw's research synthesizer. Synthesize strictly from the provided search results — " +
        ...
        try {
            ...
            return {
                ...
                keyInsights: Array.isArray(parsed.keyInsights)
                    ? parsed.keyInsights.filter((s) => typeof s === "string")
                    : [],
                openQuestions: Array.isArray(parsed.openQuestions)
                    ? parsed.openQuestions.filter((s) => typeof s === "string")
                    : [],
            };
        }
        catch {
 ...
 */
export async function synthesizeResults(query, results, llmConfig) {
