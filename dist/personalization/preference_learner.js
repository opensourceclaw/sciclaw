...
export const DEFAULT_PREFERENCE_LEARNER_CONFIG = {
    learningRate: 0.1,
...
export class PreferenceLearner {
    ...
    recordResearch(userId, topic, detail) {
    ...
    _adaptPreferences(profile) {
        ...
        profile.preferences.depth = this._clampDepth(profile.preferences.depth, depthScore);
        // Adapt style
        let styleScores = {};
        for (const e of recent) {
            const signalKey = `${e.type}-${e.detail}`;
            ...
            if (signal > 0) {
                ...
                if (e.detail.includes("paper"))
                    styleScores.academic = (styleScores.academic ?? 0) + signal;
    ...
    _clampDepth(current, score) {
        if (score > 0.15)
            return "deep";
        if (score < -0.15)
            return "shallow";
        // Small positive → stay medium or move toward deep
        if (score > 0.05 && current === "shallow")
            return "medium";
        if (score > 0.05 && current === "medium")
            return "deep";
        if (score < -0.05 && current === "deep")
            return "medium";
        if (score < -0.05 && current === "medium")
            return "shallow";
        return current;
...
/** Factory */
export function createPreferenceLearner(config) {
    return new PreferenceLearner(config);
}
//# sourceMappingURL=preference_learner.js.map