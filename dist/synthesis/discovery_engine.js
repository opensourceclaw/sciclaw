...
export class DiscoveryEngine {
    ...
    // ── Private ──────────────────────────────────────────────────────────
    _findPatterns(evidence) {
        const patterns = [];
        let idCounter = 0;
        // Pattern 1: Cross-domain term frequency
        const termDomainMap = new Map();
        for (const e of evidence) {
            for (const term of e.keyTerms) {
                const t = term.toLowerCase();
                if (!termDomainMap.has(t))
                    termDomainMap.set(t, new Set());
            ...
            for (let j = i + 1; j < evidence.length; j++) {
                ...
                if (shared.length >= this.config.minTermFrequency) {
                    patterns.push({
                        ...
                        strength: Math.min(1, shared.length / Math.max(aWords.size, bWords.size, 1) + 0.2),
    ...
    _identifyGaps(evidence, patterns) {
        const gaps = [];
        let idCounter = 0;
        ...
        for (const template of RESEARCH_TOPIC_TEMPLATES) {
            const isCovered = template.keywords.some((kw) => allText.includes(kw));
        ...
        for (const e of evidence) {
            const hasPattern = patterns.some((p) => p.domains.includes(e.domain));
    ...
    _buildSummary(patterns, gaps) {
        ...
        if (patterns.length > 0) {
            const topPatterns = patterns
                ...
                .map((p) => p.name)
                .join("; ");
            parts.push(`Discovered ${patterns.length} cross-domain patterns (top: ${topPatterns})`);
        }
        else {
        ...
        if (gaps.length > 0) {
            ...
            parts.push(`Identified ${gaps.length} research gaps (${highGaps} high priority)`);
        }
        else {
...
//# sourceMappingURL=discovery_engine.js.map