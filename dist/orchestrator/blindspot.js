...
function clusterBySimilarity(results, threshold) {
    const clusters = [];
    const used = new Set();
...
const redundancyRule = {
    name: "redundancy",
    weight: 0.4,
    detect(ctx) {
        if (ctx.results.length < 4)
            return [];
...
const timelinessRule = {
    name: "timeliness",
    weight: 0.5,
    detect(ctx) {
        const isTimeSensitive = TIME_SENSITIVE_PATTERNS.some((p) => p.test(ctx.originalQuery));
        if (!isTimeSensitive)
            return [];
...
const lowCredibilityRule = {
    ...
    detect(ctx) {
        return ctx.claims
            ...
            if (c.confidence >= 0.7)
                return false;
            if (c.contradictingSources.length > 0)
                return false;
            return c.sources.every((s) => LOW_CREDIBILITY_SOURCES.some((lcs) => s.toLowerCase().includes(lcs)));
        })
            .slice(0, 3)
            .map((c) => ({
            id: crypto.randomUUID(),
...
export class BlindSpotDetector {
    ...
    generateCompletenessReport(context) {
        ...
        const coverageRatio = totalAspects > 0 ? aspectsCovered.length / totalAspects : 0;
        // Depth assessment
        const depthPerAspect = {};
    ...
    deduplicate(spots) {
        ...
        for (const spot of spots) {
            const similar = deduped.some((d) => jaccardSimilarity(d.aspect, spot.aspect) > 0.8 &&
...
export function createBlindSpotDetector() {
    return new BlindSpotDetector();
}
//# sourceMappingURL=blindspot.js.map