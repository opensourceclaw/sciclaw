...
export class ConfidenceCalibrator {
    ...
    recordFeedback(claimId, humanJudgment, systemConfidence) {
        this.records.push({
            claimId,
            humanJudgment,
            systemConfidence,
    ...
    getCalibration() {
        ...
        for (const record of this.records) {
            ...
            if (systemVerified && !record.humanJudgment)
                falsePositives++;
            else if (!systemVerified && record.humanJudgment)
                falseNegatives++;
            else if (systemVerified && record.humanJudgment)
                truePositives++;
            else
        ...
        return {
            ...
            samplesNeeded: Math.max(0, this.minSamplesForAdjustment - this.records.length),
        };
    }
    /**
    ...
    computeOptimalThreshold() {
        ...
        for (let t = 0.4; t <= 0.95; t += 0.05) {
            ...
            for (const record of this.records) {
                ...
                if (verified && !record.humanJudgment)
                    fp++;
                else if (!verified && record.humanJudgment)
                    fn++;
                else if (verified && record.humanJudgment)
                    tp++;
                else
...
export function createConfidenceCalibrator(minSamples) {
