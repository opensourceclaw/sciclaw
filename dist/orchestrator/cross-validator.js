...
// ── Source credibility ────────────────────────────────────────────────
function getSourceCredibility(source, config) {
...
export class CrossValidator {
    config;
    constructor(config = DEFAULT_CONFIDENCE_CONFIG) {
        this.config = config;
    }
            ...
            for (const pattern of CLAIM_PATTERNS) {
                ...
                while ((match = pattern.exec(text)) !== null) {
                    ...
                    if (claimText &&
                        ...
                        const isDuplicate = claims.some((c) => c.source === result.source &&
    ...
    validate(rawClaims, results) {
        ...
        for (const cluster of clusters) {
            ...
            const contradictingSources = this.findContradictingSources(mainClaim.text, results, sources);
            const confidence = this.computeConfidence(sources, contradictingSources, allSourceSnippets);
            let status;
    ...
    findContradictingSources(claimText, results, supportingSources) {
        const contradictory = [];
        const negationPatterns = /\b(not|no|never|incorrect|false|wrong|unlikely|disputed|debunked|myth)\b/i;
        const claimKeywords = new Set(claimText.toLowerCase().split(/\W+/).filter((w) => w.length > 3));
        for (const result of results) {
            if (supportingSources.includes(result.source))
                continue;
            const text = `${result.title} ${result.snippet}`.toLowerCase();
            const hasNegation = negationPatterns.test(text);
            const keywordOverlap = [...claimKeywords].filter((kw) => text.includes(kw)).length;
    ...
    computeConfidence(supportingSources, contradictingSources, snippets) {
        const totalMentions = supportingSources.length + contradictingSources.length;
        // Agreement score: ratio of agreeing sources
        const agreementScore = totalMentions > 0 ? supportingSources.length / totalMentions : 0;
        // Source credibility: average weight of supporting sources
        const avgCredibility = supportingSources.length > 0
            ? supportingSources.reduce((sum, s) => sum + getSourceCredibility(s, this.config), 0) / supportingSources.length
            : 0.4;
        // Freshness: proxy via snippet length diversity (real impl would use dates)
        const freshnessScore = Math.min(snippets.length / 5, 1);
        const confidence = this.config.agreementWeight * agreementScore +
    ...
    findContradictions(claims) {
        ...
        for (let i = 0; i < claims.length; i++) {
            for (let j = i + 1; j < claims.length; j++) {
                ...
                const sharedSources = claimA.sources.filter((s) => claimB.sources.includes(s));
...
export function createCrossValidator(config) {
