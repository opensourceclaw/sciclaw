...
export class PreviewGenerator {
    generate(results, totalSections) {
    ...
    estimateRemaining(completed, total, completedResults) {
    ...
    generateSummary(results) {
        ...
        if (completed.length === 0)
            return 'No sections completed yet.';
        const parts = completed.map((r) => `- ${r.section.title}: ${r.wordCount} words (confidence: ${Math.round(r.confidence * 100)}%)`);
        if (failed.length > 0) {
