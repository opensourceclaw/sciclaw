...
export class ConfidenceEvaluator {
    evaluate(evidence, chainLength, sourceAuthority) {
        ...
        // Overall
        const overall = Math.round((evidenceScore + consistencyScore + authorityScore + recencyScore) * 100) / 100;
        return {
            evidenceCount,
            evidenceConsistency,
            chainLength,
    ...
    evaluateFromResults(results) {
        ...
        const avgConfidence = completed.reduce((sum, r) => sum + r.confidence, 0) / completed.length;
        // Consistency: low variance in confidence
        const variance = completed.reduce((sum, r) => sum + Math.pow(r.confidence - avgConfidence, 2), 0) /
            completed.length;
        const evidenceConsistency = Math.max(0, 1 - variance);
        const overall = Math.round((avgConfidence * 0.5 + evidenceConsistency * 0.3 + 0.5 * 0.2) * 100) / 100;
        return {
