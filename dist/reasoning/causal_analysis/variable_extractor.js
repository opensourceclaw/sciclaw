...
export class VariableExtractor {
    extract(text) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            const hasCausalKeyword = CAUSAL_KEYWORDS.some((kw) => seg.toLowerCase().includes(kw));
            for (const word of words) {
                const cleaned = word.replace(/^[^a-zA-Z\u4e00-\u9fff]+|[^a-zA-Z\u4e00-\u9fff]+$/g, '');
                if (!cleaned || cleaned.length < 2)
                    continue;
                if (/^(the|a|an|is|are|was|were|has|have|had|do|does|did|will|would|can|could|may|might|shall|should|这|那|的|了|是|在|有|和|就|不|人|都|而|及|与|着|或|一个|没有|我们|他们|它们|这个|那个|这些|那些)$/i.test(cleaned))
                    continue;
...
//# sourceMappingURL=variable_extractor.js.map