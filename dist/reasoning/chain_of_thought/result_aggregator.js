...
export class ResultAggregator {
    aggregate(results, originalQuestion, strategy = DecompositionStrategy.BALANCED) {
        const now = new Date();
        if (results.length === 0) {
            return {
                id: generateId(),
        ...
        const successfulSteps = sorted.filter((r) => r.step.status === StepStatus.COMPLETED);
        const failedSteps = sorted.filter((r) => r.step.status === StepStatus.FAILED);
        let finalResult;
        if (successfulSteps.length === 0) {
            ...
            const parts = successfulSteps.map((r) => `Step ${r.step.stepNumber}: ${r.reasoning}`);
            if (failedSteps.length > 0) {
                parts.push(`(Note: ${failedSteps.length} step(s) failed)`);
            }
            finalResult = parts.join('\n\n');
        }
        const completedSteps = sorted.filter((r) => r.step.status !== StepStatus.PENDING && r.step.status !== StepStatus.SKIPPED);
        return {
            id: generateId(),
            originalQuestion,
            strategy,
...
//# sourceMappingURL=result_aggregator.js.map