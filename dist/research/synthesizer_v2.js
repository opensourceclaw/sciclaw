...
export class LLMSynthesizer {
    ...
    tryCreateLLMEngine() {
        try {
            ...
            this.llmEngine = new OpenClawModelAdapter();
        }
        catch {
    ...
    async synthesize(request) {
        try {
            ...
            for (const theme of themes.slice(0, request.maxSections)) {
                const section = await this.synthesizeSection(request.topic, theme, request.findings, request.language);
                if (section) {
     ...
     */
    async extractThemes(topic, findings) {
        if (!this.llmEngine) {
...
Return ONLY valid JSON, no other text.`;
        try {
            const response = await this.llmEngine.chatSimple(prompt, "You are a research assistant that analyzes and categorizes information into themes. Always respond with valid JSON.");
            // Parse JSON response
     ...
     */
    extractThemesFallback(findings) {
        ...
        for (let i = 0; i < findings.length; i++) {
            ...
            const normalizedKey = themeKey.length < 10 ? `theme_${Math.floor(i / 3)}` : themeKey;
     ...
     */
    async synthesizeSection(topic, theme, allFindings, language) {
        // Get related findings
        const relatedIndices = theme.relatedFindings;
        const relatedFindings = relatedIndices
            .filter((i) => i < allFindings.length)
            .map((i) => allFindings[i])
        ...
        const findingsContent = relatedFindings
            .map((f, i) => `[${i + 1}] ${f.content.slice(0, 300)}`)
            .join("\n\n");
...
Return ONLY valid JSON.`;
     ...
     */
    async generateSummary(topic, sections, language) {
 ...
 */
export async function synthesizeFindings(topic, findings, options = {}) {
...
//# sourceMappingURL=synthesizer_v2.js.map