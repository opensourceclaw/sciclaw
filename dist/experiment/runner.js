...
function measureMetric(metric, _variables, stepResults) {
...
export class ExperimentRunner {
    ...
    async run(experiment) {
        ...
        if (experiment.status === ExperimentStatus.FAILED ||
            ...
            throw new Error(`Cannot run experiment in ${experiment.status} state`);
        }
        ...
        const sortedSteps = [...experiment.steps].sort((a, b) => a.order - b.order);
        for (const step of sortedSteps) {
            ...
            if (result.status === "passed") {
                observations.push(`Step ${step.order}: ${step.action} - ${result.actualOutcome}`);
            }
            else {
                observations.push(`Step ${step.order}: ${step.action} - FAILED: ${result.errors.join(", ")}`);
            }
        }
        // Measure metrics
        const metricResults = experiment.metrics.map((m) => measureMetric(m, experiment.variables, stepResults));
    ...
    getRunnerStats() {
        return {
            ...
            avgDurationMs: this.totalRun > 0
                ? Math.round(this.totalDuration / this.totalRun)
                : 0,
        };
    }
}
export function createExperimentRunner(config) {
