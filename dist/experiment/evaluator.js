...
function generateRecommendations(report) {
    ...
    if (report.overallScore > 0.8) {
        recs.push("Results are conclusive; consider publication or deployment");
    }
    if (report.overallScore < 0.4) {
        recs.push("Redesign experiment with tighter controls and more variables");
    }
    if (report.stepSummary.skipped > 0) {
        recs.push("Address dependency failures to prevent step skipping");
    }
    return recs;
}
// ── ExperimentEvaluator ──────────────────────────────────────────────────
export class ExperimentEvaluator {
    ...
    evaluate(result, hypothesis) {
        ...
        const metricPassed = result.metricResults.filter((m) => m.passed).length;
        const metricFailed = result.metricResults.length - metricPassed;
        const stepsPassed = result.stepResults.filter((s) => s.status === "passed").length;
        const stepsFailed = result.stepResults.filter((s) => s.status === "failed").length;
        const stepsSkipped = result.stepResults.filter((s) => s.status === "skipped").length;
        const metricSummary = {
            total: result.metricResults.length,
            passed: metricPassed,
            failed: metricFailed,
            passRate: result.metricResults.length > 0
                ? Math.round((metricPassed / result.metricResults.length) * 100) / 100
                : 0,
        };
        const stepSummary = {
            ...
            completionRate: result.stepResults.length > 0
                ? Math.round((stepsPassed / result.stepResults.length) * 100) / 100
                : 0,
        };
        const hypothesisSupported = overallScore >= this.config.minConfidenceForSupport;
...
export function createExperimentEvaluator(config) {
