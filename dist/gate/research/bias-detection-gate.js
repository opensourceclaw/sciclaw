...
export class BiasDetectionGate {
    ...
    async check(context) {
        ...
        const score = assessed.length > 0 ? assessed.reduce((sum, d) => sum + d.score, 0) / assessed.length : 0;
        ...
        if (!structurallyCorroborated) {
            ...
            recommendations.push(`insufficient_corroboration: fewer than ${MIN_UNIQUE_URLS} unique sources — a single source cannot corroborate itself`);
        }
        return result;
    }
    checkSelectionBias(context, uniqueUrls) {
        if (uniqueUrls === 0) {
            return {
                item: "selection",
                score: 1,
