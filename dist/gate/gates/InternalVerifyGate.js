...
export class InternalVerifyGate {
    ...
    async submit(submission) {
        ...
        if (!validation.allPassed) {
            ...
            throw new Error(`Internal verify validation failed: ${validation.failedGates.join(", ")}`);
        }
    ...
    validate(submission) {
        ...
        const summary = this.buildSummary(failedGates, warnings, submission.tests, submission.regression);
        return {
    ...
    // ── Private Helpers ─────────────────────────────────────────────
    buildSummary(failedGates, warnings, tests, regression) {
        const parts = [];
...
//# sourceMappingURL=InternalVerifyGate.js.map