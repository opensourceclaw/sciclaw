...
export class ReportGenerator {
  ...
  private sectioner: SmartSectioner;
   ...
   */
  private generateMarkdown(result: SynthesisResult): string {
    const lines: string[] = [];
    ...
    // Metadata
    lines.push(
      `**Generated:** ${this.formatDate(result.createdAt)}`
    );
   ...
   */
  private generateHtml(result: SynthesisResult): string {
    ...
    htmlParts.push(
      `  <p class='metadata'>Generated: ${this.formatDate(result.createdAt)}</p>`
    );
    ...
    if (this.config.includeToc && result.sections.length > 0) {
      ...
      for (let i = 0; i < result.sections.length; i++) {
        ...
        const anchor = this.slugify(section.title);
        htmlParts.push(
          `      <li><a href="#${anchor}">${escapeHtml(section.title)}</a></li>`
        );
      }
      htmlParts.push("    </ul>");
      htmlParts.push("  </nav>");
    ...
    for (const section of result.sections) {
      ...
      if (this.config.includeSources && section.sources.length > 0) {
        ...
        for (const source of section.sources) {
          htmlParts.push(
            `        <li>${this.formatCitationHtml(source)}</li>`
          );
        }
        htmlParts.push("      </ol>");
        htmlParts.push("    </div>");
      ...
      const confClass =
        section.confidence >= 0.7
          ...
          : section.confidence >= 0.4
            ...
            : "low";
      htmlParts.push(
        `    <p class='confidence'>Confidence: <span class='badge ${confClass}'>${this.formatPercent(section.confidence)}</span></p>`
      );
    ...
    // Footer
    htmlParts.push(
      `  <footer>Report generated by SciClaw v${result.version}</footer>`
    );
    htmlParts.push("  </div>");
   ...
   */
  private generateJson(result: SynthesisResult): string {
    const data = {
      topic: result.topic,
   ...
   */
  private generateErrorReport(result: SynthesisResult): string {
    const lines: string[] = [
      `# Error: ${result.topic}`,
      "",
      "## Status",
   ...
   */
  private formatCitationWithScore(source: Finding): string {
    const citation = this.formatCitation(source);
    if (source.qualityScore) {
      const qs = source.qualityScore;
      let indicator: string;
      if (qs >= 0.8) indicator = " [High Quality]";
      else if (qs >= 0.6) indicator = " [Good]";
      else if (qs >= 0.4) indicator = " [Average]";
      else indicator = " [Low Quality]";
   ...
   * Format citation
   */
  private formatCitation(source: Finding): string {
   ...
   */
  private formatCitationHtml(source: Finding): string {
    const escapeHtml = (str: string): string =>
      str
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
   ...
   * Save report to file
   */
  async save(result: SynthesisResult, outputPath: string): Promise<void> {
  ...
  private confidenceIndicator(confidence: number): string {
...
export function generateReport(
  ...
  format: ReportConfig["format"] = "markdown",
  options: Partial<ReportConfig> = {}
): string {
  const config: ReportConfig = { ...DEFAULT_REPORT_CONFIG, ...options, format };
