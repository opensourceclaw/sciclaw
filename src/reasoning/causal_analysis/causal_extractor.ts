...
import type { CausalVariable, CausalRelation } from './types.js';
...
function detectDirection(relation: string): 'positive' | 'negative' | 'unknown' {
...
function matchVariable(text: string, variables: CausalVariable[]): string | null {
  const lower = text.trim().toLowerCase();
  if (!lower) return null;

  for (const v of variables) {
    if (lower.includes(v.name.toLowerCase())) return v.id;
    for (const alias of v.aliases) {
      if (lower.includes(alias.toLowerCase())) return v.id;
    }
  }
  return null;
}

function calculateConfidence(evidenceText: string, allEvidence: string[]): number {
...
export class CausalExtractor {
  extract(text: string, variables: CausalVariable[]): CausalRelation[] {
    if (!text || !text.trim() || variables.length === 0) return [];
