...
import type { CausalVariable, CausalRelation, CausalGraph, ImpactPath, ImpactAnalysis } from './types.js';
...
export class GraphBuilder {
  ...
  detectCycles(relations: CausalRelation[]): string[][] {
    ...
    for (const rel of relations) {
      if (!adjList.has(rel.sourceId)) adjList.set(rel.sourceId, []);
  ...
  analyzeImpact(relationId: string, variables: CausalVariable[], relations: CausalRelation[]): ImpactAnalysis {
    ...
    const adjList = new Map<string, CausalRelation[]>();
    for (const rel of relations) {
      if (!adjList.has(rel.sourceId)) adjList.set(rel.sourceId, []);
    ...
    const queue: Array<{ nodeId: string; path: CausalRelation[]; visited: Set<string> }> = [
    ...
    while (queue.length > 0) {
      ...
      for (const rel of outgoing) {
        if (current.visited.has(rel.targetId)) continue;
  ...
  findConnectedComponents(variables: CausalVariable[], relations: CausalRelation[]): CausalGraph[] {
    ...
    const adjList = new Map<string, string[]>();
    for (const v of variables) adjList.set(v.id, []);
    for (const rel of relations) {
      adjList.get(rel.sourceId)?.push(rel.targetId);
