...
function topologicalSort(steps: ReasoningStep[]): ReasoningStep[] {
  const stepMap = new Map(steps.map((s) => [s.id, s]));
  ...
  for (const step of steps) {
    for (const depId of step.dependsOn) {
      ...
      if (neighbors) neighbors.push(step.id);
    }
  }
...
export class ReasoningChainManager {
  ...
  constructor(config?: Partial<ReasoningConfig>) {
    ...
    if (this.config.maxSteps > 20) this.config.maxSteps = 20;
    if (this.config.minConfidence < 0.1) this.config.minConfidence = 0.1;
    if (this.config.minConfidence > 0.9) this.config.minConfidence = 0.9;
  }
  ...
  addStep(
    ...
    if (chain.steps.length >= chain.maxSteps) {
      throw new Error(
        `Max steps (${chain.maxSteps}) exceeded for chain ${chain.id}`,
      );
    }
  ...
  validateChain(chain: ReasoningChain): { valid: boolean; errors: string[] } {
    ...
    if (chain.steps.length < 5) {
      errors.push(
        `Chain must have at least 5 steps, got ${chain.steps.length}`,
      );
    }
  ...
  executeChain(
    ...
    const computedSteps: ReasoningStep[] = sorted.map((step) => {
    ...
    const conclusionStep = computedSteps.find(
      (s) => s.type === ReasoningStepType.CONCLUSION,
    );
    ...
    const alternativeConclusions = computedSteps
      .filter(
        (s) =>
          s.type === ReasoningStepType.HYPOTHESIS ||
