...
type QuestionType = 'factual' | 'causal' | 'procedural' | 'comparison' | 'multi';

function classifyQuestion(question: string): QuestionType {
  const q = question.toLowerCase().trim();

  if (q.includes('?') && (q.match(/\?/g)?.length ?? 0) > 1) return 'multi';

  if (/^(what|who|when|where|哪个|谁|什么|什么时候|哪里)/.test(q)) return 'factual';
  if (/^(why|how\s+(come|is|does)|为什么|怎么(会|能|可能))/i.test(q)) return 'causal';
  if (/^(how\s+(to|do|can|should)|步骤|如何|怎样)/i.test(q)) return 'procedural';
  if (/(compare|difference|vs|versus|区别|对比|不同|比较)/i.test(q)) return 'comparison';

  // Fallback: check for multiple clauses
  if (/[,，;；、]/.test(q) || /(and|与|和|以及|同时)/.test(q)) return 'multi';
...
function getMaxChildren(strategy: DecompositionStrategy): number {
  switch (strategy) {
...
export class ProblemDecomposer {
  ...
  private decomposeCausal(
    ...
    for (let i = 0; i < subSteps.length; i++) {
      if (steps.length >= this.config.maxSteps) break;
  ...
  private decomposeProcedural(
    ...
    for (let i = 0; i < subSteps.length; i++) {
      if (steps.length >= this.config.maxSteps) break;
      const id = this.makeStep(
        steps.length,
        subSteps[i]!,
        rootId,
        strategy,
        1,
        i === 0 ? [rootId] : [rootId, prevId],
      );
      steps.push(id);
      prevId = id.id;
    }
