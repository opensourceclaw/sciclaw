...
import type { LLMEngine, ChatMessage, MessageRole, ChatOptions } from './types.js';
...
const MAX_TOKENS: Record<SummarizationLength, number> = {
...
export class Summarizer {
  ...
  extractiveSummarize(content: string, length: SummarizationLength): SummaryResult {
    ...
    let n: number;
    if (length === SummarizationLength.SHORT) n = 2;
    else if (length === SummarizationLength.MEDIUM) n = 5;
    else n = 10;
