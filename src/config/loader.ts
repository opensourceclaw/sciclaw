 ...
 */
export async function loadConfigFile(cwd: string = process.cwd()): Promise<ConfigFile | null> {
...
async function tryLoadFile(path: string): Promise<ConfigFile | null> {
  try {
    await access(path);
...
export function mergeConfig(
  ...
  envConfig: Partial<DeepClawConfig>
): DeepClawConfig {
  return {
    defaultEngine: (envConfig.defaultEngine as any) ??
      (fileConfig?.search?.defaultProvider as any) ??
      'duckduckgo',
    maxResults: envConfig.maxResults ?? fileConfig?.search?.maxResults ?? 50,
    timeout: envConfig.timeout ?? fileConfig?.search?.timeout ?? 30000,
    outputFormat: (envConfig.outputFormat as any) ??
      (fileConfig?.output?.format as any) ??
      'markdown',
    llm: envConfig.llm,
    api: {
 ...
 */
export async function loadFullConfig(cwd?: string): Promise<DeepClawConfig> {
  const fileConfig = await loadConfigFile(cwd);
  const envConfig: Partial<DeepClawConfig> = {};

  // Load from environment
  if (process.env.DEEPCLAW_ENGINE) {
    envConfig.defaultEngine = process.env.DEEPCLAW_ENGINE as any;
  }
  ...
  if (process.env.DEEPCLAW_OUTPUT_FORMAT) {
    envConfig.outputFormat = process.env.DEEPCLAW_OUTPUT_FORMAT as any;
  }
