...
function runNpmScript(script: string, cwd: string): { status: number; output: string } {
...
export function parseVitestSummary(output: string): TestResult {
  const line = output.match(/^\s*Tests\s+(.+)$/m)?.[1] ?? "";
  const num = (label: string): number => Number(line.match(new RegExp(`(\\d+) ${label}`))?.[1] ?? 0);
...
export function scanQuality(srcDir: string): QualityChecks {
  ...
  for (const file of walkTs(srcDir)) {
    ...
    if (!hasChineseChars && /[\u4E00-\u9FFF]/.test(text)) hasChineseChars = true;
    if (!hasHardcodedPaths && /\/Users\/|\/home\/|[A-Z]:\\/.test(text)) hasHardcodedPaths = true;
    if (!hasMissingApacheHeaders) {
      const header = text.split("\n").slice(0, 15).join("\n");
      if (!/apache|license/i.test(header)) hasMissingApacheHeaders = true;
    }
    if (hasChineseChars && hasHardcodedPaths && hasMissingApacheHeaders) break;
  }
 ...
 */
export function collectVerifySubmission(pipelineId: string, cwd: string = process.cwd()): InternalVerifySubmission {
  const version = (
    JSON.parse(fs.readFileSync(path.join(cwd, "package.json"), "utf-8")) as { version: string }
  ).version;
...
function readPreviousTestCount(): number | null {
  try {
