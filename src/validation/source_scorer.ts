import type { DomainScore, FreshnessScore, SourceScore, AuthorityScore } from "./types.js";

const KNOWN_DOMAINS: Record<string, number> = {
  "wikipedia.org": 0.85,
  "arxiv.org": 0.95,
...
function hasTld(domain: string, tlds: string[]): boolean {
  ...
  scoreDomain(url: string): DomainScore {
    ...
    for (const [knownDomain, score] of Object.entries(KNOWN_DOMAINS)) {
      if (domain === knownDomain || domain.endsWith("." + knownDomain)) {
        reputation = score;
        if (score >= 0.85) category = "academic";
        else if (score >= 0.7) category = "news";
        else if (score >= 0.5) category = "tech";
        else category = "low_trust";
...
export class SourceScorer {
  ...
  scoreSource(url: string, pubDate?: string, authorityScore?: number): SourceScore {
    ...
    const authority: AuthorityScore = { score: authorityScore ?? 0.5, hasCredentials: false };
