...
function hasCycleInSteps(steps: ExperimentStep[]): boolean {
...
function getExperimentType(category: string): ExperimentType {
  switch (category) {
    case "causal":
...
function extractVariables(statement: string): ExperimentVariable[] {
  ...
  const nounPhrases = statement.match(
...
function generateSteps(): ExperimentStep[] {
...
function generateMetrics(type: ExperimentType): ExperimentMetric[] {
  switch (type) {
...
export class ExperimentDesigner {
  ...
  private totalDesigned = 0;

  constructor(config?: Partial<ExperimentConfig>) {
    this.config = { ...DEFAULT_EXPERIMENT_CONFIG, ...config };
    if (this.config.maxSteps < 3) this.config.maxSteps = 3;
    if (this.config.maxSteps > 100) this.config.maxSteps = 100;
    if (this.config.maxVariables < 2) this.config.maxVariables = 2;
    if (this.config.maxVariables > 50) this.config.maxVariables = 50;
  }
  ...
  design(hypothesis: {
    ...
    const variables = extractVariables(hypothesis.statement).slice(
      0,
      this.config.maxVariables,
    );
    const steps = generateSteps();
    const metrics = generateMetrics(type);
    const constraints: ExperimentConstraint[] = [
      {
        id: generateId(),
  ...
  classifyVariable(name: string, statement: string): VariableType {
    if (statement.startsWith(name)) return VariableType.INDEPENDENT;
