...
export class RiskScorer {
  private sourceWeight: number;
  private claimWeight: number;
  ...
  assessRisk(claim: Claim, sourceScore: number, verificationScore = 0.5): RiskAssessment {
    ...
    const overallRisk =
      (1 - sourceScore) * this.sourceWeight +
  ...
  getRiskLevel(score: number): RiskLevel {
    if (score >= 0.65) return RiskLevel.CRITICAL;
    if (score >= 0.45) return RiskLevel.HIGH;
    if (score >= 0.25) return RiskLevel.MEDIUM;
    return RiskLevel.LOW;
  }

  private generateWarnings(claim: Claim, sourceScore: number, verificationScore: number, level: RiskLevel): string[] {
    const warnings: string[] = [];
    if (level === RiskLevel.CRITICAL) warnings.push("Untrusted source detected — verify independently");
    else if (level === RiskLevel.HIGH) warnings.push("Low-quality source — cross-reference recommended");
    if (sourceScore < 0.4) warnings.push(`Source reliability is low (${sourceScore.toFixed(2)})`);
    if (verificationScore < 0.3) warnings.push("Claim has not been verified");
    if (claim.type === ClaimType.OPINION) warnings.push("Subjective claim — may not be factual");
    if (claim.type === ClaimType.CAUSATION) warnings.push("Causation claim — verify causal relationship");
    if (!claim.sourceUrl) warnings.push("Missing source attribution");
    return warnings;
  }

  private generateRecommendations(claim: Claim, sourceScore: number, level: RiskLevel): string[] {
    const recs: string[] = [];
    if (level === RiskLevel.HIGH || level === RiskLevel.CRITICAL) {
      recs.push("Seek confirmation from at least 2 independent sources");
    }
    if (sourceScore < 0.4) recs.push("Replace with more reliable source if available");
    if (claim.type === ClaimType.NUMERIC) recs.push("Verify numbers against original source data");
    if (claim.type === ClaimType.CAUSATION) recs.push("Check for correlation vs causation fallacy");
    return recs;
  }
}
