...
export class FactCheckService {
  ...
  private cache: Map<string, { result: VerificationResult; expiresAt: number }> = new Map();

  constructor(cacheTTLMinutes = 30) {
    this.cacheTTL = cacheTTLMinutes * 60 * 1000;
  }

  async verifyClaim(claim: Claim, referenceSources: string[] = []): Promise<VerificationResult> {
  ...
  async verifyClaims(claims: Claim[], referenceSources: string[] = []): Promise<VerificationResult[]> {
    return Promise.all(claims.map((c) => this.verifyClaim(c, referenceSources)));
  }

  async verifySource(url: string, claims: Claim[]): Promise<SourceVerificationSummary> {
  ...
  private verifyInternal(claim: Claim, referenceSources: string[]): VerificationResult {
    if (!claim.text) {
  ...
  private evaluate(claim: Claim, supporting: number, contradicting: number): VerificationResult {
  ...
  private hasSimilarNumbers(text1: string, text2: string): boolean {
  ...
  private cacheKey(claim: Claim): string {
    const raw = `${claim.text}:${claim.type}:${claim.position}`;
    return crypto.createHash("sha256").update(raw).digest("hex").slice(0, 16);
  }
}

export async function verifyClaim(claim: Claim, referenceSources?: string[]): Promise<VerificationResult> {
  const service = new FactCheckService();
  return service.verifyClaim(claim, referenceSources);
}

export async function verifySource(url: string, claims: Claim[]): Promise<SourceVerificationSummary> {
  const service = new FactCheckService();
