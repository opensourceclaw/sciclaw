...
export class RichConsole {
   ...
   */
  print(message: string, style: string = "info", level?: OutputLevel): void {
   ...
   */
  printPanel(content: string, title?: string, style: string = "info"): void {
    if (!this.config.useColors) {
   ...
   */
  createSpinner(description: string = "Processing..."): Ora {
    return ora({
      text: description,
      spinner: "dots",
      color: "cyan",
...
let globalConsole: RichConsole | null = null;

/**
 * Get global console instance
 */
export function getConsole(config?: CLIConfig): RichConsole {
