...
export function isRetryableError(error: Error): boolean {
  ...
  if (networkErrorCodes.includes((error as NodeJS.ErrnoException).code || '')) {
    return true;
  }
...
export async function retryWithBackoff<T>(
  ...
  for (let attempt = 0; attempt < config.maxAttempts; attempt++) {
    try {
      ...
      console.warn(
        `Attempt ${attempt + 1}/${config.maxAttempts} failed: ${lastError.message}. ` +
...
export async function retry<T>(
  ...
  const config: RetryConfig = {
    maxAttempts: options?.maxAttempts ?? defaultRetryConfig.maxAttempts,
    initialDelay: options?.initialDelay ?? (options as any)?.delay ?? 100,
    maxDelay: options?.maxDelay ?? defaultRetryConfig.maxDelay,
    exponentialBase: options?.exponentialBase ?? defaultRetryConfig.exponentialBase,
