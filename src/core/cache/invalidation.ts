...
import type { InvalidationConfig, InvalidationStrategy } from "./types.js";
...
export class InvalidationManager {
  private topicWindows = new Map<string, number>();

  constructor(private config: InvalidationConfig = { ...DEFAULT_INVALIDATION_CONFIG }) {}
  ...
  private computeTopicTTL(
    params: { query: string; accessCount: number; lastAccessGapMs: number },
    base: number,
  ): number {
  ...
  private computeSourceTTL(params: { engines: SearchEngine[] }, base: number): number {
    let ttl = base;
  ...
  private computeSourceFactor(engines: SearchEngine[]): number {
    let factor = 1.0;
