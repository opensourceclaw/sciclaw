...
export interface TopicTrackerConfig {
  decayRate: number; // per-day interest decay
  minWeight: number; // below this, remove from tracking
  maxTopics: number; // max tracked topics
  boostFactor: number; // weight boost per research
  relatedTopicsBoost: number; // boost for related topics
}

export const DEFAULT_TOPIC_TRACKER_CONFIG: TopicTrackerConfig = {
  decayRate: 0.005,
  minWeight: 0.1,
  maxTopics: 20,
  boostFactor: 0.15,
...
export class TopicTracker {
  private config: TopicTrackerConfig;
  ...
  recordResearch(topic: string): TopicInterest {
    ...
    if (interest) {
      interest.frequency++;
      interest.weight = Math.min(
        1,
        interest.weight + this.config.boostFactor,
      );
  ...
  applyDecay(): void {
    ...
    for (const [topic, interest] of this.interests) {
      const daysSince =
        (now - interest.lastResearched.getTime()) / (1000 * 60 * 60 * 24);
  ...
  /** Get top N interests */
  getTopInterests(n: number = 5): TopicInterest[] {
  ...
  suggestTopics(n: number = 3): string[] {
    const researched = new Set(this.interests.keys());
    const candidates = new Map<string, number>(); // topic → score

    for (const interest of this.interests.values()) {
      for (const related of interest.relatedTopics) {
        if (researched.has(related)) continue;
        const score =
          (candidates.get(related) ?? 0) + interest.weight;
  ...
  getDiversityScore(): number {
    this.applyDecay();
    if (this.interests.size <= 1) return 0;

    const weights = Array.from(this.interests.values()).map(
      (i) => i.weight,
    );
  ...
  private _findRelated(topic: string): string[] {
    // Direct lookup
    if (TOPIC_RELATIONS[topic]) return [...TOPIC_RELATIONS[topic]];
  ...
  private _boostRelated(topic: string): void {
    const interest = this.interests.get(topic);
    if (interest) {
      interest.weight = Math.min(
        1,
        interest.weight + this.config.relatedTopicsBoost,
      );
    }
  }

  private _enforceMaxTopics(): void {
    if (this.interests.size <= this.config.maxTopics) return;

    const sorted = Array.from(this.interests.entries()).sort(
      (a, b) => a[1].weight - b[1].weight, // ascending, lowest first
    );

    const toRemove = sorted.slice(
      0,
      this.interests.size - this.config.maxTopics,
    );
...
export function createTopicTracker(
  config?: Partial<TopicTrackerConfig>,
): TopicTracker {
