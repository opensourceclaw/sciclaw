...
const CONNECTION_PATTERNS: Array<{
  type: DomainConnection["type"];
  patterns: RegExp[];
}> = [
  {
    type: "causation",
...
export class CrossDomainSynthesizer {
  ...
  private _findConnections(evidence: DomainEvidence[]): DomainConnection[] {
  ...
  private _connectPair(
    ...
    // Shared key terms
    const sharedTerms = a.keyTerms.filter(
      (t) =>
        b.keyTerms.some((u) => u.toLowerCase() === t.toLowerCase()) &&
        t.length >= 3,
    );
    ...
    const termOverlap =
      sharedTerms.length / Math.max(a.keyTerms.length + b.keyTerms.length, 1);
    const wordOverlap =
      overlap.length / Math.max(aWords.size + bWords.size, 1);
  ...
  private _extractInsights(
    ...
    for (const e of evidence) {
      for (const term of e.keyTerms) {
        ...
        if (!allTerms.has(t)) allTerms.set(t, new Set());
  ...
  private _buildInsightStatement(
    ...
    const verbMap: Record<DomainConnection["type"], string> = {
  ...
  private _computeOverallConfidence(
    ...
    if (connections.length === 0 && insights.length === 0) return 0;
    const connAvg =
...
export function createCrossDomainSynthesizer(
