...
export class SourceCredibilityGate implements ResearchGate {
  ...
  private scoreSource(result: { url: string; timestamp: string; relevanceScore: number }): number {
  ...
  private scoreDomain(domain: string): number {
    if (HIGH_CRED.some(d => domain.includes(d))) return 0.95;
    if (MEDIUM_CRED.some(d => domain.includes(d))) return 0.75;
  ...
  private recommend(details: GateDetail[]): string[] {
    ...
    if (lowScore.length > 0) return [`Consider replacing ${lowScore.length} low-credibility sources`];
    return [];
  }
}
