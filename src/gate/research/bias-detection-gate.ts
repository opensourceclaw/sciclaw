...
export class BiasDetectionGate implements ResearchGate {
  ...
  private recommendMitigations(details: GateDetail[]): string[] {
    ...
    if (selection && selection.score > 0.3) recs.push("Diversify information sources");
    const confirmation = details.find(d => d.item === "confirmation");
    if (confirmation && confirmation.score > 0.3) recs.push("Add counter-arguments to strengthen analysis");
    return recs;
  }
}
