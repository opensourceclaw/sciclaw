...
function emptyTable(format: TableFormat): TableData {
  return {
    headers: [],
    rows: [],
    sourceFormat: format,
...
function parseHTMLTable(html: string): TableData[] {
  ...
  for (const tableHTML of matches) {
    ...
    const headerMatches = tableHTML.match(
      /<th[^>]*>([\s\S]*?)<\/th>/gi,
    );
...
function parseCSVTable(csv: string): TableData {
  ...
  if (lines.length === 0) return emptyTable(TableFormat.CSV);
...
function parseJSONTable(json: string): TableData {
  try {
    const parsed = JSON.parse(json);
    const arr = Array.isArray(parsed) ? parsed : [parsed];

    if (arr.length === 0) return emptyTable(TableFormat.JSON);
...
function detectFormat(input: string): TableFormat {
  if (/<table[\s>]/i.test(input)) return TableFormat.HTML;
  if (/^\|[\s\-:]+\|/m.test(input)) return TableFormat.MARKDOWN;
  try {
    JSON.parse(input);
...
export class TableExtractor {
  private config: MultiModalConfig["table"];
  private totalExtracted = 0;
  private formatDist: Record<string, number> = {};

  constructor(config?: Partial<MultiModalConfig["table"]>) {
    this.config = { ...DEFAULT_MULTIMODAL_CONFIG.table, ...config };
    if (this.config.maxRows < 1) this.config.maxRows = 1;
    if (this.config.maxRows > 10000) this.config.maxRows = 10000;
    if (this.config.maxColumns < 1) this.config.maxColumns = 1;
    if (this.config.maxColumns > 100) this.config.maxColumns = 100;
  }
  ...
  extractTable(input: string, format?: TableFormat): TableData {
    ...
    if (table.headers.length > this.config.maxColumns) {
      ...
      table.rows = table.rows.map((r) =>
        r.slice(0, this.config.maxColumns),
      );
  ...
  validateTable(table: TableData): { valid: boolean; errors: string[] } {
    ...
    for (let i = 0; i < table.rows.length; i++) {
      if (table.rows[i]!.length !== table.headers.length) {
        errors.push(
          `Row ${i + 1} has ${table.rows[i]!.length} columns, expected ${table.headers.length}`,
        );
      }
    }
