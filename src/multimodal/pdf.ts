...
function extractPDFMetadata(text: string): PDFMetadata {
  ...
  const creationDateMatch = text.match(
    /\/CreationDate\s*\(([^)]*)\)/,
  );
...
function exportText(pdfContent: PDFContent): string {
...
export class PDFAnalyzer {
  private config: MultiModalConfig["pdf"];
  ...
  constructor(config?: Partial<MultiModalConfig["pdf"]>) {
    ...
    if (this.config.maxPages < 1) this.config.maxPages = 1;
    if (this.config.maxPages > 10000) this.config.maxPages = 10000;
  }
  ...
  exportPageText(pdfContent: PDFContent, pageNumber: number): string {
    const page = pdfContent.pages.find(
  ...
  async validatePDF(
    ...
    if (buffer.length > this.config.maxFileSizeBytes) {
      errors.push(
        `PDF exceeds max size (${buffer.length} > ${this.config.maxFileSizeBytes})`,
      );
    }
