...
import { ImageFormat, ContentCategory, ChartType } from "./types.js";
...
export const DEFAULT_GEMINI_VISION_CONFIG: GeminiVisionConfig = {
...
export class GeminiVisionAdapter {
  ...
  constructor(
    adapterConfig?: Partial<{ baseUrl: string; apiKey?: string; timeoutMs: number }>,
...
Respond in JSON format:
{
  ...
  async interpretChart(image: Buffer): Promise<ChartData> {
...
Respond in JSON:
{
  ...
  "confidence": 0.0-1.0
    ...
    if (
      ...
      this.proFallbackCount++;
      return this.parseChartResult(
        await this.callVision("gemini-2.5-pro", image, prompt),
      );
    }
  ...
  async readDiagram(image: Buffer): Promise<DiagramData> {
...
Respond in JSON:
{
  ...
  async extractScreenshot(image: Buffer): Promise<ScreenshotData> {
...
Respond in JSON:
{
  ...
  private parseCategory(raw: string): ContentCategory {
  ...
  private parseChartType(raw: string): ChartType {
...
export function createGeminiVisionAdapter(
  adapterConfig?: Partial<{ baseUrl: string; apiKey?: string; timeoutMs: number }>,
  visionConfig?: Partial<GeminiVisionConfig>,
): GeminiVisionAdapter {
