...
function pickTerms(evidence: Evidence[]): string[] {
  const terms = new Set<string>();
  for (const e of evidence) {
    const kt = extractKeyTerms(e.content);
    for (const t of kt.slice(0, 3)) terms.add(t);
  }
  const arr = [...terms];
...
function determineCategory(evidence: Evidence[]): HypothesisCategory {
...
export class HypothesisGenerator {
  ...
  generate(
    evidence: Evidence[],
    context?: { topic?: string; domain?: string },
  ): Hypothesis[] {
    ...
    for (const cluster of clusters) {
      if (cluster.length < this.config.minEvidencePerHypothesis) continue;
      ...
      const avgRelevance =
        cluster.reduce((s, e) => s + e.relevance, 0) / cluster.length;
      const avgReliability =
        cluster.reduce((s, e) => s + e.reliability, 0) / cluster.length;
    ...
    while (hypotheses.length < this.config.minHypotheses && evidence.length > 0) {
      ...
      if (hypotheses.length >= this.config.maxHypotheses) break;
    }
  ...
  refineHypothesis(
    ...
    if (confidence >= 0.7) status = HypothesisStatus.SUPPORTED;
    else if (confidence < 0.3) status = HypothesisStatus.REFUTED;
