/**
 * SciClaw v3.0.0 — Hypothesis Ranker
 *
...
function normalizeWeights(weights: RankingWeights): RankingWeights {
  const sum =
    ...
    weights.testability +
    weights.coherence;
  if (sum === 0) return { ...DEFAULT_HYPOTHESIS_CONFIG.rankingWeights };
  return {
...
function computeNovelty(h: Hypothesis, all: Hypothesis[]): number {
...
function computeTestability(statement: string): number {
  ...
  if (statement.includes("?")) score -= 0.2;
  return Math.min(Math.max(score, 0), 1.0);
}

function computeCoherence(h: Hypothesis): number {
  let score = 0.7;
  const stmt = h.statement.toLowerCase();
  if (h.supportingEvidence.length > 0 && h.contradictingEvidence.length === 0)
    score += 0.15;
  if (h.supportingEvidence.length >= 3) score += 0.1;
  if (stmt.length < 10) score -= 0.3;
  if (stmt.length > 500) score -= 0.1;
  return Math.min(Math.max(score, 0), 1.0);
}

function strengthLabel(score: number): EvidenceStrength {
  if (score >= 0.8) return EvidenceStrength.STRONG;
  if (score >= 0.5) return EvidenceStrength.MODERATE;
  if (score >= 0.3) return EvidenceStrength.WEAK;
...
export class HypothesisRanker {
  ...
  rank(hypotheses: Hypothesis[]): RankedHypothesis[] {
    ...
    const scored = hypotheses.map((h, _idx, all) => {
      ...
      const score =
        this.weights.evidenceSupport * evidenceSupport +
