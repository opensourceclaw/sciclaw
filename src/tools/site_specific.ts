...
export class SiteParser {
  ...
  static REQUIRES_JS: boolean = false;
  ...
  static can_handle(url: string): boolean {
    if (!this.DOMAINS || this.DOMAINS.length === 0) return false;
...
export class GitHubParser extends SiteParser {
  ...
  parse($: cheerio.CheerioAPI, url: string): ParsedContent {
    const result: ParsedContent = { title: '', content: '' };
    ...
    if (time) result.date = time;

    // README content
    const readme = $('.markdown-body').text().trim();
    if (readme) result.content = readme;
...
export class ZhihuParser extends SiteParser {
  ...
  parse($: cheerio.CheerioAPI, url: string): ParsedContent {
    ...
    if (content.length) result.content = content.text().trim();
...
export class StackOverflowParser extends SiteParser {
  ...
  parse($: cheerio.CheerioAPI, url: string): ParsedContent {
    ...
    if (post.length) result.content = post.text().trim();
...
export class HackerNewsParser extends SiteParser {
  ...
  parse($: cheerio.CheerioAPI, url: string): ParsedContent {
    ...
    if (comment.length) result.content = comment.text().trim();
...
export class VergeParser extends SiteParser {
  ...
  parse($: cheerio.CheerioAPI, url: string): ParsedContent {
    ...
    if (article.length) result.content = article.text().trim();
 ...
 */
export const SITE_PARSERS: typeof SiteParser[] = [
...
const PARSER_REGISTRY: Map<string, typeof SiteParser> = new Map();

/**
 * Register a new parser dynamically
 */
export function registerParser(parserClass: typeof SiteParser): void {
 ...
 */
export function unregisterParser(domain: string): void {
 ...
 */
export function getParser(url: string): SiteParser | null {
 ...
 */
export function parseSite(url: string, html: string): ParsedContent {
...
export default {
  SiteParser,
  SiteParserConfig: {} as SiteParserConfig,
  SITE_PARSERS,
  getParser,
  canHandle,
  parseSite,
