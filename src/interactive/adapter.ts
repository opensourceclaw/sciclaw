...
function createDefaultContext(topic: string): SessionContext {
  return {
    topic,
    depth: 0,
    focus: [topic],
...
function applyActionToContext(
  ...
  switch (action) {
    ...
    case FeedbackAction.MODIFY:
      return {
        ...context,
        focus: context.focus.map((f) =>
          f === context.focus[0] ? content : f,
        ),
      };
    default:
      return context;
...
export class HumanInLoopAdapter {
  ...
  constructor(config?: Partial<InteractiveConfig>) {
    ...
    if (this.config.maxTurns < 5) this.config.maxTurns = 5;
    if (this.config.maxTurns > 200) this.config.maxTurns = 200;
  }
  ...
  processInput(
    ...
    session.context = applyActionToContext(
      input.action,
      input.content,
      session.context,
    );
  ...
  getActiveSessions(): InteractionSession[] {
    return [...this.sessions.values()].filter(
      (s) => s.status === SessionStatus.ACTIVE || s.status === SessionStatus.WAITING_USER,
    );
  }
