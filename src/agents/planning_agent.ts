...
function generateQueries(topic: string, subtopics: string[], strategy: string): string[] {
  if (strategy === "broad") {
...
function extractSubtopics(topic: string): string[] {
  if (!topic || topic.length === 0) return [];
  if (topic.length > 500) topic = topic.slice(0, 500);
...
export class PlanningAgent extends BaseAgent {
  ...
  async execute(task: AgentTask): Promise<AgentResult> {
    const input = task.input as { topic?: string; depth?: string; context?: Record<string, unknown> } | undefined;
