# ResearchClaw - Technical Architecture Design
...
```
User → OpenClaw → ResearchClaw Skill → Results
```
...
```
User → OpenClaw → neoclaw Plugin → Full ResearchClaw
```
...
```
User → ResearchClaw API → Full System
```
...
|-----------|--------|----------|
| Research Reports | Markdown (.md) | `~/.researchclaw/data/research/` |
| Metadata | JSON | `~/.researchclaw/metadata/` |
| Search Cache | JSON | `~/.researchclaw/cache/` |
| User Config | JSON | `~/.researchclaw/config.json` |
...
# Metadata
YYYY-MM-DD_topic-slug.json

# Cache
cache_{query_hash}.json
```
...
---
*Generated by ResearchClaw v{version}*
```
