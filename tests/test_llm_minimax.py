...
class TestMiniMaxProvider:
    ...
    def provider(self):
        ...
        return MiniMaxProvider(api_key="test-api-key")
    ...
    def test_supported_models(self, provider):
        ...
        assert "abab6.5s-chat" in provider.supported_models
        assert "abab6.5g-chat" in provider.supported_models
        assert "abab5.5s-chat" in provider.supported_models
    ...
    def test_custom_base_url(self):
        ...
        provider = MiniMaxProvider(api_key="test", base_url="https://custom.example.com")
    ...
    def test_chat_stream(self, mock_post, provider):
        ...
        mock_response.iter_lines.return_value = [
            b'data: {"id":"minimax-123","choices":[{"delta":{"role":"assistant","content":"\xe4\xbd\xa0"},"finish_reason":null}]}',
            b'data: {"id":"minimax-123","choices":[{"delta":{"content":"\xe5\xa5\xbd"},"finish_reason":"stop"}]}',
            b"data: [DONE]",
        ]
        mock_response.raise_for_status = MagicMock()
        mock_post.return_value = mock_response
